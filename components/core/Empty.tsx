...
const Empty = ({ title, subTitle, pathName, link }: { title: string; subTitle?: string; pathName: string; link: string }) => {
    return (
        <div className=" flex flex-col gap-5">
            ...
            <div>
                <h2 className="text-center text-gray-700 dark:text-gray-200 text-base font-semibold leading-relaxed pb-1">{title}</h2>
                <p className="text-center text-gray-700 dark:text-gray-200 text-sm font-normal leading-snug pb-4">{subTitle}</p>
                <div className="flex gap-3">
                    <Link href={link} className="w-full text-center py-2 bg-indigo-600 hover:bg-indigo-700 transition-all duration-500 rounded-full text-white text-xs font-semibold leading-4">{pathName}</Link>
                </div>
            </div>
        </div>
    )
